...
namespace Area
{
    static class ClipboardPasteOperation
    {
        ...
        static void TrickleDown (TrickleDownSpec spec)
        {
            ...
            var levelBottom = am.boundsFull.y;
            ...
            for (var index = pointCount - gridSize; index < pointCount; index += 1)
            {
                ...
                if (am.IsPointTerrain (point))
                {
                    AreaSceneHelper.ClearTileData (point);
                    if (spec.Log)
                    {
                ...
                for (var z = 0; z < spec.AffectedBounds.z; z += 1)
                {
                    ...
                    for (var x = 0; x < spec.AffectedBounds.x; x += 1)
                    {
                        ...
                        if (indexNeighborUp >= 0)
                        {
                            ...
                            if (am.IsPointTerrain (neighborUp) && ((point.spotConfiguration & TilesetUtility.configurationTopMask) >> 4 == (neighborUp.spotConfiguration & TilesetUtility.configurationFloor)))
                            {
        ...
        static int UpdateHolePointState (PunchOutSpec spec)
        {
            ...
            while (anchorIndex >= 0)
            {
                ...
                UpdatePointStateOnPaste (am, sourceOrigin, sources, am.points.Count, log);
                anchorIndex -= layerSize;
                yStop -= 1;
            }
