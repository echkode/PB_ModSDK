...
namespace Area
{
    ...
    sealed class VolumeRemoveOperation : AreaSceneVolumeOperation
    {
        ...
        bool UpdateBlocks ()
        {
            ...
            foreach (var point in pointsToEdit)
            {
                ...
                for (var i = 0; i < spots.Length; i += 1)
                {
                    ...
                    else if (AreaSceneHelper.IsSpotInterior (spot))
                    {
                        ...
                        terrainModifiedPoint |= ConvertInteriorSpot (am, spot, faceSpot, bb.enableVolumeLogging);
                    }
                    else if (bb.overrideTerrainAndRoadTilesetsOnVolumeEdits && am.IsPointTerrain (spot))
                    {
                        terrainModifiedPoint |= ChangeTileset (spot);
                    }
                ...
                if (terrainModifiedPoint)
                {
                    ...
                    VolumeOperationHelper.TrickleDown (am, point, bb.volumeTilesetSelected.id, layerSize,  modifiedIndexes, bb.enableVolumeLogging);
                }
            }
        ...
        bool ChangeTileset (AreaVolumePoint spot)
        {
            ...
            var changeTerrain = bb.am.IsPointTerrain (spot) || selectedID == AreaTilesetHelper.idOfTerrain;
            if (bb.enableVolumeLogging)
            {
                Debug.LogFormat ("Change tileset | index: {0} | tileset: {1} --> {2} | terrain: {3}", spot.spotIndex, spot.blockTileset, selectedID, changeTerrain);
            }
            AreaSceneHelper.ClearTileData (spot);
            spot.blockTileset = selectedID;
            modifiedIndexes.Add (spot.spotIndex);
            return changeTerrain;
        }

        static bool ConvertInteriorSpot (AreaManager am, AreaVolumePoint spot, AreaVolumePoint faceSpot, bool log)
        {
            var isTerrain = am.IsPointTerrain (faceSpot);
        ...
        bool UpdateSpotConfiguration (AreaVolumePoint spot, int neighborIndex, bool log)
        {
            ...
            if (clearTileset)
            {
                ...
                changeTerrain = bb.am.IsPointTerrain (spot);
                AreaSceneHelper.ClearTileData (spot);
            }
            ...
            if (emptyState && spot.pointState != AreaVolumePointState.Empty)
            {
                ...
                changeTerrain = changeTerrain || bb.am.IsPointTerrain (spot);
