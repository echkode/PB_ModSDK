...
namespace Area
{
    ...
    sealed class VolumeAddOperation : AreaSceneVolumeOperation
    {
        ...
        bool UpdateBlocks ()
        {
            ...
            foreach (var point in pointsToEdit)
            {
                ...
                for (var i = point.pointsWithSurroundingSpots.Length - 1; i >= 0; i -= 1)
                {
                    ...
                    {
                        var changeTerrain = am.IsPointTerrain (spot);
                        if (bb.enableVolumeLogging)
                        {
                    ...
                    terrainModifiedPoint |= UpdateSpotConfiguration (am, spot, i, bb.enableVolumeLogging);
                }
                if (terrainModifiedPoint)
                {
                    terrainModified = true;
                    VolumeOperationHelper.TrickleDown (am, point, selectedTilesetID, layerSize,  modifiedIndexes, bb.enableVolumeLogging);
                }
            }
        ...
        static bool CheckTilesetOverride (AreaSceneBlackboard bb, AreaVolumePoint avp, int selectedID)
        {
            ...
            return bb.overrideTerrainAndRoadTilesetsOnVolumeEdits && AreaSceneHelper.VolumePointHasTerrainOrRoadTilesets (bb.am, avp);
        }

        static bool UpdateSpotConfiguration (AreaManager am, AreaVolumePoint spot, int neighborIndex, bool log)
        {
            ...
            {
                changeTerrain = am.IsPointTerrain (spot);
