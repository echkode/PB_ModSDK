...
namespace Area
{
    ...
    {
        public static void TrickleDown (AreaManager am, AreaVolumePoint point, int selectedTilesetID, int layerSize, List<int> modifiedIndexes, bool log)
        {
            trickleDownExcludes.Clear ();
            point = point.pointsInSpot[WorldSpace.Compass.Down];
            while (point != null)
            {
                if (log)
                {
