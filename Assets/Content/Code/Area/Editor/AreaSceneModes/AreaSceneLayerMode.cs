...
namespace Area
{
    ...
    sealed class AreaSceneLayerMode : AreaSceneMode
    {
        ...
        {
            var offset = AreaUtility.spotOffset;
            offset.y = 0f;
            hitPoint.y = layer * -TilesetUtility.blockAssetSize;
    ...
    sealed class AreaSceneLayerModeFloor
    {
        ...
        float FloorVertical () => (currentLayer + 1) * -TilesetUtility.blockAssetSize - floorOffset;
        ...
        (float, float) InstantiateLayerFloor ()
        {
            ...
            var sizeX = (bounds.x - 1) * TilesetUtility.blockAssetSize;
            var sizeZ = (bounds.z - 1) * TilesetUtility.blockAssetSize;
        ...
        void BuildGridLines (float sizeX, float sizeZ)
        {
            ...
            var lineIndex = 0;
            var y = FloorVertical ();
            var x = TilesetUtility.blockAssetSize;
            for (var i = 1; i < boundsX; i += 1, x += TilesetUtility.blockAssetSize)
            {
                gridlines[lineIndex++] = new Vector3 (x, y, 0f);
                gridlines[lineIndex++] = new Vector3 (x, y, sizeZ);
            }

            var z = TilesetUtility.blockAssetSize;
            for (var i = 1; i < boundsZ; i += 1, z += TilesetUtility.blockAssetSize)
            {
        ...
        void InstantiateCollider (float sizeX, float sizeZ)
        {
            ...
            bc.size = new Vector3 (sizeX, TilesetUtility.blockAssetSize, sizeZ);
            collider.transform.localPosition = new Vector3 (sizeX / 2f, ColliderVertical (), sizeZ / 2f);

            if (visualTransform != null)
            {
                var scale = bc.size;
                scale.y = TilesetUtility.blockAssetSize;
        ...
        float ColliderVertical () => currentLayer * -TilesetUtility.blockAssetSize - TilesetUtility.blockAssetHalfSize;
    ...
    sealed class AreaSceneLayerModeOperations
    {
        ...
        bool SetTileset (AreaVolumePoint cell)
        {
            ...
            var terrainModified = bb.am.IsPointTerrain (cell) || tileset == AreaTilesetHelper.idOfTerrain;
            cell.blockTileset = tileset;
            if (tileset == 0)
            {
                AreaSceneHelper.ClearTileData (cell);
            }
