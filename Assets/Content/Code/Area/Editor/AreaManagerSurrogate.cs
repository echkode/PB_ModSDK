    ...
    sealed class AreaManagerSurrogate
    {
        ...
        public void GenerateProps ()
        {
            ...
            foreach (var point in am.points)
            {
                ...
                if (point.blockTileset != AreaTilesetHelper.idOfForest && !am.IsPointTerrain (point))
                {
                    continue;
                }
        ...
        public void RegenerateNavGraph () => AreaNavUtility.GetNavigationNodes (ref navNodes, bb.am);

        List<IAreaNavNode> navNodes;

        [HorizontalGroup (OdinGroup.Name.OtherButtonsRow1, Width = 95f)]
        [Button ("Regenerate\nnav overrides", ButtonHeight = 40)]
        public void RegenerateNavOverrides () => bb.am.GenerateNavOverrides ();
        ...
        void GenerateRamps (AreaManager.SlopeProximityCheck proximityCheck)
        {
            ...
            var filePath = DataPathHelper.GetCombinedCleanPath (dirTextureMaps.FullName, Heightmap.standardHeightmapFileName);
        ...
        public void CreateStarterHeightmap ()
        {
            ...
            Heightmap.Create (bb.am, BasinHeightmap.CalculateValues, filePath);
        ...
        public void ImportDepthFromStarter ()
        {
            ...
            var filePath = DataPathHelper.GetCombinedCleanPath (dirTextureMaps.FullName, starterHeightmapFileName);
            Heightmap.ImportHeightFromTexture (bb.am, filePath);
            Heightmap.ImportRoadsFromTexture (bb.am, filePath);
        }
        ...
        public void ExportDepthRampRoadToTexture ()
        {
            ...
            var filePath = DataPathHelper.GetCombinedCleanPath (dirTextureMaps.FullName, Heightmap.standardHeightmapFileName);
            Heightmap.Create (bb.am, Heightmap.CalculateStandardHeightmapValues, filePath);
            heightmapExported = true;
        }
        ...
        public void ImportDepthFromTexture () => ImportFeatureFromTexture (Heightmap.ImportHeightFromTexture);

        void ImportFeatureFromTexture (System.Action<AreaManager, string> importFeature)
        {
            if (!dirTextureMaps.Exists)
            {
                return;
            ...
            var filePath = DataPathHelper.GetCombinedCleanPath (dirTextureMaps.FullName, Heightmap.standardHeightmapFileName);
            importFeature (bb.am, filePath);
        }

        [HorizontalGroup (OdinGroup.Name.HeightmapButtonsRow3)]
        [PropertySpace (2f)]
        [Button ("Import\nramps", ButtonHeight = 40)]
        public void ImportRampsFromTexture () => ImportFeatureFromTexture (Heightmap.ImportRampsFromTexture);

        [HorizontalGroup (OdinGroup.Name.HeightmapButtonsRow3)]
        [PropertySpace (2f)]
        [Button ("Import\nroads", ButtonHeight = 40)]
        public void ImportRoadsFromTexture () => ImportFeatureFromTexture (Heightmap.ImportRoadsFromTexture);
            ...
            for (var y = 0; y < bounds.y; y += 1)
            {
                ...
                for (var z = 0; z < bounds.z; z += 1)
                {
                    for (var x = 0; x < bounds.x; x += 1)
                    {
                        ...
                        else if (am.IsPointTerrain(point))
                        {
