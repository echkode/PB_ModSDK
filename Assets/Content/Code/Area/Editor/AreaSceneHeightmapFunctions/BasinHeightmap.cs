...
namespace Area
{
    public static class BasinHeightmap
    {
        ...
        public static void CalculateValues (Heightmap.Spec spec)
        {
            ...
            FillSouthWestQuadrant (spec.AreaManager, spec.SizeX, spec.SizeZ, grid, lowest, spec.Store);
            FillNorthWestQuadrant (spec.AreaManager, spec.SizeX, spec.SizeZ, grid, lowest, spec.Store);
            FillNorthEastQuadrant (spec.AreaManager, spec.SizeX, spec.SizeZ, grid, lowest, spec.Store);
            FillSouthEastQuadrant (spec.AreaManager, spec.SizeX, spec.SizeZ, grid, lowest, spec.Store);
        }
        ...
        static void CopyBufferZone (Heightmap.Spec spec, int[,] grid)
        {
            ...
            for (var z = 0; z < keepSteps; z += 1)
            {
                for (var x = 0; x < sizeX; x += 1)
                {
                    ...
                    store (spec.AreaManager, x, z, depth, byte.MinValue, road ? byte.MaxValue : byte.MinValue);
                }
            }
        ...
        static void FillSouthWestQuadrant (AreaManager am, int sizeX, int sizeZ, int[,] grid, int lowest, Heightmap.StoreHeightmapInfo store)
        {
            var halfX = sizeX / 2;
            var halfZ = sizeZ / 2;
            for (var z = keepSteps; z < halfZ; z += 1)
            {
                for (var x = keepSteps; x < halfX; x += 1)
                {
                    ...
                    grid[x, z] = v;
                    store(am, x, z, v, byte.MinValue, byte.MinValue);
                }
            }
        }

        static void FillNorthWestQuadrant (AreaManager am, int sizeX, int sizeZ, int[,] grid, int lowest, Heightmap.StoreHeightmapInfo store)
        {
            var lastZ = sizeZ - 1;
            var halfX = sizeX / 2;
            var halfZ = sizeZ / 2;
        ...
        static void FillNorthEastQuadrant (AreaManager am, int sizeX, int sizeZ, int[,] grid, int lowest, Heightmap.StoreHeightmapInfo store)
        {
            var lastX = sizeX - 1;
            var lastZ = sizeZ - 1;
            var halfX = sizeX / 2;
        ...
        static void FillSouthEastQuadrant (AreaManager am, int sizeX, int sizeZ, int[,] grid, int lowest, Heightmap.StoreHeightmapInfo store)
        {
            var lastX = sizeX - 1;
            var halfX = sizeX / 2;
            var halfZ = sizeZ / 2;
